...
public class WrapString {
	/**
	   ...
	   */
	  public static List<String> wrap(String str, Paint fm, int maxWidth) {
	   ...
	   */
	  public static void wrapLineInto(String line, List<String> list, Paint fm, int maxWidth) {
	    int len = line.length();
	    int width;
	    while (len > 0 && (width = (int) fm.measureText(line)) > maxWidth) {
	      // Guess where to split the line. Look for the next space before
	      // or after the guess.
	      int guess = len * maxWidth / width;
	      String before = line.substring(0, guess).trim();

	      width = (int) fm.measureText(before);
	      int pos;
	      if (width > maxWidth) // Too long
	        ...
	        if (pos != -1) { // Make sure this doesn't make us too long
	          ...
	          if (fm.measureText(before) > maxWidth)
	            pos = findBreakBefore(line, guess);
	        }
	      }
	      if (pos == -1)
